...
var usageTemplate = template.Must(template.New("usage").Parse(`Multirun is a tool for running of github workflows with tests for different repositories.

Usage:

	multirun command [arguments]

The commands are:
{{ range $name, $cmd := .Commands }}
	{{ printf "%-10s" $name }} {{ $cmd.ShortDescription }}
{{ end }}
Use "multirun help [command]" for more information about a command.
`))
